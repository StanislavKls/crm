...
class ModuleMake extends Command
{
    ...
    private function createMigration()
    {
        ...
        try {
            $this->call('make:migration', [
                ...
                '--create' => $table,
                '--path'   => "app/Modules/" . str_replace("\\", DIRECTORY_SEPARATOR, trim($this->argument('name'))) . "/Migrations" ,
            ]);
        } catch (\Exception $e) {
    ...
    {
        return base_path('resources/js/components/' . str_replace('\\', DIRECTORY_SEPARATOR, $name) . ".vue");
    }
    ...
    protected function getViewPath($name): object
    {
        ...
        $paths = $arrFiles->map(function ($item) use ($name) {
            return base_path('resources/views/' . str_replace('\\', DIRECTORY_SEPARATOR, $name) . DIRECTORY_SEPARATOR . $item . ".blade.php");
        });
    ...
    private function getControllerPath($argument)
    {
        ...
        return $this->laravel['path'] . '/Modules/' . str_replace("\\", DIRECTORY_SEPARATOR, $argument) . "/Controllers/" . "{$controller}Controller.php";
    }
    private function getApiControllerPath($name)
    {
        $controller = Str::studly(class_basename($name));
        return $this->laravel['path'] . '/Modules/' . str_replace("\\", DIRECTORY_SEPARATOR, $name) . "/Controllers/Api/" . "{$controller}Controller.php";
    }
    ...
    {
        return $this->laravel['path'] . '/Modules/' . str_replace('\\', DIRECTORY_SEPARATOR, $name) . "/Routes/web.php";
    }
    ...
    {
        return $this->laravel['path'] . '/Modules/' . str_replace('\\', DIRECTORY_SEPARATOR, $name) . "/Routes/api.php";
    }
